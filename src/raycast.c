...
void	first_step(t_ray *rey, float *r_pos, int r)
{
	if (r == x)
	{
		...
		else
		{
			...
			if (rey->ray[x].cross[y] <= 0 || rey->ray[x].cross[y] > 5)
				rey->ray[x].cross[y] = r_pos[y];
			else
	...
	else
	{
		...
		else
		{
			...
			if (rey->ray[y].cross[x] <= 0 || rey->ray[y].cross[x] > 5)
				rey->ray[y].cross[x] = r_pos[x];
			else
...
void	raycast(t_cub *cub, double ray_ang, int *i)
{
	...
	rey.ray[x].step[y] = fabs(1 * tan(rey.ang));
	rey.ray[y].step[x] = fabs(1 / tan(rey.ang));
